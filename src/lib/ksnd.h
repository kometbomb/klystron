 ...
 *
 * @param player the @c KPlayer context you will use to play the song
 * @param data[in] pointer to data containing the song
 * @param data_size size of data pointed by @c pointer
 * @return @c KSong or @c NULL if there was an error.
 */
 ...
 *
 * If @c NULL is passed as the @c data pointer, an internal, thread-unsafe buffer will
 ...
 *
 * @param[in] pointer to a @c KSongInfo structure, or @c NULL
 * @return pointer to a @c KSongInfo structure containing the information
 */
 ...
 * from the beginning to pattern row @c position. Use this in conjunction with KSND_GetPlayPosition()
 * to find out the current playback time in milliseconds.
 *
